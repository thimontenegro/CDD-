...
respostas %>%
    ...
    ggplot(aes(x = NOME_CURSO, y = media)) +
...
respostas %>%
    filter(categoria == "Infraestrutura") %>%
    ggplot(aes(x = reorder(NOME_CURSO, media, sd), y = media)) +
    geom_jitter(width = .1, alpha = .2) +
    labs(
        x = "UF do curso",
        y = "Avaliação numa escala de 1 a 6",
...
respostas %>%
    filter(categoria == "Infraestrutura") %>%
    ggplot(aes(x = reorder(NOME_CURSO, media, max), y = media)) +
    geom_jitter(width = .1, alpha = .2) +
    labs(
        x = "UF do curso",
        y = "Avaliação numa escala de 1 a 6",
...
respostas %>%
    filter(categoria == "Infraestrutura") %>%
    ggplot(aes(x = reorder(NOME_CURSO, media), y = media)) +
    geom_point(
        data = filter(medias, categoria == "Infraestrutura"),
        color = "orange",
        shape = 15,
...
departamentos %>%
    filter(categoria != "Outros") %>%
    ggplot(aes(x = reorder(tema, media), y = media, color = depto)) +
